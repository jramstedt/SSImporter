...
public static class CharacterControllerUtilities
{
        ...
        NativeList<StatefulCollisionEvent> collisionEvents = default)
    {
        ...
        for (int i = 0; i < castHitsCollector.NumHits; ++i)
        {
            ...
            CreateConstraint(physicsWorld, stepInput.DeltaTime, stepInput.Up,
    ...
    private static void CastColliderAndCreateConstraints(CharacterControllerStepInput stepInput, float remainingTime, ColliderCastInput input, float maxSlopeCos, ref NativeList<SurfaceConstraintInfo> constraints, out float minHitFraction, NativeList<ColliderCastHit> triggerHits = default) {
        ...
        float3 displacement = input.End - input.Start;
        ...
        minHitFraction = collector.MinHitFraction;
    }

    private static void CalculateDistanceAndFixPenetratingHits(CharacterControllerStepInput stepInput, float remainingTime, ColliderDistanceInput input, float maxSlopeCos, ref NativeList<SurfaceConstraintInfo> constraints) {
        PhysicsWorld world = stepInput.PhysicsWorldSingleton.PhysicsWorld;
                ...
                for (int constraintIndex = numConstraints - 1; constraintIndex >= 0; --constraintIndex)
                {
                        ...
                        constraint.ColliderKey.Equals(hit.ColliderKey))
                    {
                        // Fix up the constraint (normal, distance)
                        {
                            ...
                            ResolveConstraintPenetration(ref newConstraint, remainingTime);
    ...
    public static void CollideAndIntegrate(
        ...
        ref RigidTransform bodyTransform,
        ref float3 bodyVelocity,
        
        ref PhysicsCollider headCollider,
        ref RigidTransform headTransform,
        ref float3 headVelocity,
        float3 headOffset,
        ...
        NativeList<StatefulCollisionEvent> collisionEvents = default, NativeList<StatefulTriggerEvent> triggerEvents = default)
    {
        ...
        float3 newPosition = bodyTransform.pos;
        quaternion orientation = bodyTransform.rot;
        float3 newVelocity = bodyVelocity;

        float3 newHeadPosition = headTransform.pos;
        quaternion headOrientation = headTransform.rot;
        float3 newHeadVelocity = bodyVelocity + math.mul(bodyTransform.rot, headVelocity);
        ...
        {
            NativeList<SurfaceConstraintInfo> bodyConstraints = new NativeList<SurfaceConstraintInfo>(k_DefaultConstraintsCapacity, Allocator.Temp);
            NativeList<SurfaceConstraintInfo> headConstraints = new NativeList<SurfaceConstraintInfo>(k_DefaultConstraintsCapacity, Allocator.Temp);
            ...
            // Do a collider cast
            {
                ...
                var minHitFraction = float.MaxValue;
                
                { // Body
                    float3 bodyDisplacement = newVelocity * remainingTime;
                    ColliderCastInput input = new ColliderCastInput(bodyCollider.Value, newPosition, newPosition + bodyDisplacement, orientation);
                    CastColliderAndCreateConstraints(stepInput, remainingTime, input, maxSlopeCos, ref bodyConstraints, out minHitFraction, triggerHits);
                }
                
                // Update trigger events
                if (triggerEvents.IsCreated)
                {
                    UpdateTriggersSeen(stepInput, triggerHits, triggerEvents, minHitFraction);
                }

                { // Head
                    float3 headDisplacement = newHeadVelocity * remainingTime;
                    ColliderCastInput input = new ColliderCastInput(headCollider.Value, newHeadPosition, newHeadPosition + headDisplacement, headOrientation);
                    CastColliderAndCreateConstraints(stepInput, remainingTime, input, maxSlopeCos, ref headConstraints, out minHitFraction);
                }
            }
            ...
            {
                { // Body
                    ColliderDistanceInput input = new ColliderDistanceInput(bodyCollider.Value, stepInput.ContactTolerance, bodyTransform);
                    CalculateDistanceAndFixPenetratingHits(stepInput, remainingTime, input, maxSlopeCos, ref bodyConstraints);
                }

                { // Head
                    ColliderDistanceInput input = new ColliderDistanceInput(headCollider.Value, stepInput.ContactTolerance, headTransform);
                    CalculateDistanceAndFixPenetratingHits(stepInput, remainingTime, input, maxSlopeCos, ref headConstraints);
                }
            }

            // Min delta time for solver to break
            float minDeltaTime = 0.0f;
            if (math.lengthsq(newHeadVelocity) > k_SimplexSolverEpsilonSq)
            {
                // Min delta time to travel at least 1cm
                minDeltaTime = 0.01f / math.length(newHeadVelocity);
            }

            // Solve
            float3 prevVelocity = newVelocity;
            float3 prevPosition = newPosition;
            float3 prevHeadPosition = newHeadPosition;
            
            NativeList<SurfaceConstraintInfo> allConstraints = new NativeList<SurfaceConstraintInfo>(bodyConstraints.Length + headConstraints.Length, Allocator.Temp);
            allConstraints.AddRangeNoResize(bodyConstraints);
            allConstraints.AddRangeNoResize(headConstraints);
            
            SimplexSolver.Solve(remainingTime, minDeltaTime, up, stepInput.MaxMovementSpeed, allConstraints, ref newPosition, ref newVelocity, out float integratedTime);
            newHeadPosition = newPosition + math.mul(bodyTransform.rot, headOffset);
            ...
            if (affectBodies || collisionEvents.IsCreated)
            {
                CalculateAndStoreDeferredImpulsesAndCollisionEvents(stepInput, affectBodies, characterMass,
                    prevVelocity, allConstraints, ref deferredImpulseWriter, collisionEvents);
            }
            ...
            bodyTransform.pos = newPosition;
            headTransform.pos = newHeadPosition;
        }
    ...
    private static void CreateMaxSlopeConstraint(float3 up, ref SurfaceConstraintInfo constraint, out SurfaceConstraintInfo maxSlopeConstraint, float deltaTime)
    {
        ...
        if (newConstraint.Plane.Distance < 0.0f)
        {
            ...
            ResolveConstraintPenetration(ref newConstraint, deltaTime);
        }
    ...
    private static void ResolveConstraintPenetration(ref SurfaceConstraintInfo constraint, float deltaTime)
    {
        // Fix up the velocity to enable penetration recovery
        if (constraint.Plane.Distance < 0.0f)
        {
            float3 newVel = constraint.Velocity - (constraint.Plane.Normal * constraint.Plane.Distance) / deltaTime;
            constraint.Velocity = newVel;
            constraint.Plane.Distance = 0.0f;
        }
    }

    private static void CreateConstraint(PhysicsWorld world, float deltaTime, float3 up,
        ...
        float skinWidth, float maxSlopeCos, ref NativeList<SurfaceConstraintInfo> constraints)
    {
        ...
        if (shouldAddPlane)
        {
            ...
            CreateMaxSlopeConstraint(up, ref constraint, out SurfaceConstraintInfo maxSlopeConstraint, deltaTime);
            constraints.Add(maxSlopeConstraint);
        }

        // Prepare velocity to resolve penetration
        ResolveConstraintPenetration(ref constraint, deltaTime);
