    ...
    public partial struct CharacterControllerSystem : ISystem
    {
        ...
        public void OnCreate(ref SystemState state)
        {
            characterControllerQuery = SystemAPI.QueryBuilder()
                .WithAllRW<HackerController, HackerControllerInternal>()
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            var ccJob = new CharacterControllerJob
            {
                ...
                CharacterControllerHandle = SystemAPI.GetComponentTypeHandle<HackerController>(true),
                CharacterControllerInternalHandle = SystemAPI.GetComponentTypeHandle<HackerControllerInternal>(),
            ...
            var smoothedCharacterControllerQuery = SystemAPI.QueryBuilder()
                .WithAllRW<HackerControllerInternal, PhysicsGraphicalSmoothing>().Build();
        ...
        {
            public void Execute(in HackerControllerInternal ccInternal,
        ...
        struct CharacterControllerJob : IJobChunk
        {
            ...
            public ComponentTypeHandle<HackerControllerInternal> CharacterControllerInternalHandle;
            ...
            [ReadOnly] public ComponentTypeHandle<HackerController> CharacterControllerHandle;
                ...
                in v128 chunkEnabledMask)
            {
                ...
                for (int i = 0; i < chunk.Count; i++)
                {
                    ...
                    Util.StepInput stepInput = new Util.StepInput
                    {
                        ...
                        RigidBodyIndex = PhysicsWorldSingleton.PhysicsWorld.GetRigidBodyIndex(entity),
            ...
            private void HandleUserInput(
                ...
                ref HackerControllerInternal controllerInternal,
                ref float3 linearVelocity)
            {
                // Reset jumping state and unsupported velocity
                if (controllerInternal.SupportedState == Util.CharacterSupportState.Supported) {
                    controllerInternal.IsJumping = false;
                    controllerInternal.UnsupportedVelocity = float3.zero;
                }
                ...
                float3 requestedMovementDirection = float3.zero;
                {
                    ...
                    float horizontal = controllerInternal.InputMoveDelta.x;
                    float vertical = controllerInternal.InputMoveDelta.y;
                    bool jumpRequested = controllerInternal.InputMoveDelta.z != 0;
                    // controllerInternal.Input.Jumped = 0; // "consume" the event
                    ...
                    if (haveInput)
                    {
                        ...
                        float3 worldSpaceMovement =
                            math.rotate(quaternion.AxisAngle(up, controllerInternal.CurrentRotationAngle),
                                localSpaceMovement);
                        requestedMovementDirection = math.normalize(worldSpaceMovement);
                    }

                    shouldJump = jumpRequested && controllerInternal.SupportedState == Util.CharacterSupportState.Supported;
                }

                // Turning
                {
                    float horizontal = controllerInternal.InputLookDelta.x;

                    bool haveInput = (math.abs(horizontal) > float.Epsilon);
                    if (haveInput)
                    {
                        var userRotationSpeed = horizontal * controller.RotationSpeed;
                        controllerInternal.Velocity.Angular = -userRotationSpeed * up;
                        controllerInternal.CurrentRotationAngle += userRotationSpeed * DeltaTime;
                    }
                    else
                    {
                        controllerInternal.Velocity.Angular = 0f;
                    }
                }
                ...
                // Apply input velocities
                {
                    if (shouldJump) // TODO jump jet
                    {
                        ...
                        controllerInternal.IsJumping = true;
                        controllerInternal.SupportedState = Util.CharacterSupportState.Unsupported;
                        controllerInternal.UnsupportedVelocity = surfaceVelocity + controller.JumpUpwardsSpeed * up;
                    }
                    else if (controllerInternal.SupportedState != Util.CharacterSupportState.Supported)
                    {
                        // Apply gravity
                        controllerInternal.UnsupportedVelocity += controller.Gravity * DeltaTime;
                    }

                    // If unsupported then keep jump and surface momentum
                    linearVelocity = requestedMovementDirection * controller.MovementSpeed +
                        (controllerInternal.SupportedState != Util.CharacterSupportState.Supported
                            ? controllerInternal.UnsupportedVelocity
                            : float3.zero);
                }
            }
